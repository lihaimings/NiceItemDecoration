...
//                final float centerTop = (layoutManager.getBottomDecorationHeight(child) + 1 - topBottom) / 2;
                ...
                //判断是否为第一排
                boolean isFirst = lookup.getSpanGroupIndex(position, spanCount) == 0;
                Log.d("位置", "position=" + position + "\tspanSize=" + spanSize + "\tspanIndex=" + spanIndex + "\tisFirst=" + isFirst);
                //绘制横行
                ...
                if (!isFirst && spanIndex == 0) {
                    ...
                    top = (int) (child.getTop()) - topBottom;
                    bottom = top + topBottom;
                    mDivider.setBounds(left, top, right, bottom);
                    mDivider.draw(c);
                }
                //绘制垂直
                //最右边的一排不需要右边的： 所在每行的位置+权重
            ...
            for (int i = 0; i < childCount; i++) {
                ...
                //绘制垂直
                if (!isFirst && spanIndex == 0) {
                    left = (int) (child.getLeft() ) - leftRight;
                    right = left + leftRight;
                ...
                if (!isRight) {
                    //计算右边的
                    left = child.getLeft();
                    right = (int) (child.getRight());
                    top = (int) (child.getBottom());
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if (layoutManager.getOrientation() == GridLayoutManager.VERTICAL) {
            ...
            if (lp.getSpanSize() == spanCount) {
                ...
                outRect.left = (int)
                        (((float) (spanCount - lp.getSpanIndex())) / spanCount * leftRight);
                Log.d("距离", "left=" + outRect.left);

                outRect.right = (int)
                        (((float) leftRight * (spanCount + 1) / spanCount) - outRect.left);
                Log.d("距离", "right=" + outRect.right);
            }
        } else {
            //水平方向
